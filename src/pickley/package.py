...
class DeliveryMethod:
    ...
    def install(self, target, source):
        ...
        """
        system.debug("deliver: %s => %s", target, source)
...
class DeliverySymlink(DeliveryMethod):
    def install(self, target, source):
        ...
        :param str source: Path to original executable being delivered (.pickley/<package>/...)
        """
        system.symlink(target, source)


@DELIVERERS.register
class DeliveryWrapper(DeliveryMethod):
    def install(self, target, source):
        """
        :param str target: Full path of executable to deliver (<base>/<entry_point>)
        :param str source: Path to original executable being delivered (.pickley/<package>/...)
        ...
        with open(target, "wt") as fh:
            ...
            cmd = [SETTINGS.base.full_path(system.PICKLEY), "-q", "install", os.path.basename(target)]
            fh.write("%s\n" % system.represented_args(cmd, shorten=False))
            cmd = ["exec", source, "$*"]
            fh.write("%s\n" % system.represented_args(cmd, shorten=False))
