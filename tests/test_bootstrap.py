...
def test_bootstrap(cli, monkeypatch):
    ...
    with patch("pickley.bstrap.os.path.expanduser", side_effect=mocked_expanduser):
        ...
        cli.run("-n -cfoo --check-path", main=bstrap.main)
        assert cli.failed
        ...
        #     assert cli.succeeded
        #     assert "uv -q pip install --target=.local/bin/.pk/pickley-1.0 pickley==1.0" in cli.logged
